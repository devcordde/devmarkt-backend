...
public abstract class UserRepo implements CrudRepository<User, Integer> {
    ...
    var sql = """
        INSERT INTO user_roles
            SELECT u.id, r.id FROM users u
              LEFT JOIN roles r ON r.name = ANY (?)
            WHERE u.id_type = ? AND u.user_id = ?;
        """;
    operations.prepareStatement(sql, statement -> {
      statement.setArray(1, roleNameSqlArray(roleNames));
  ...
  public void removeRoles(UserId id, Collection<String> roleNames) {
    ...
    operations.prepareStatement(sql, statement -> {
      ...
      statement.setArray(3, roleNameSqlArray(roleNames));
      return statement.executeUpdate();
    });
  }

  private Array roleNameSqlArray(Collection<String> roleNames) throws SQLException {
    return operations.getConnection().createArrayOf("VARCHAR", roleNames.toArray());
  }
}
